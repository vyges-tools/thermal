...
fn analog_pa_output_stage_is_the_hotspot() {
    ...
    assert!((rep.hot_x_um - 300.0).abs() < 60.0, "hotspot x {}", rep.hot_x_um);
    assert!((rep.hot_y_um - 300.0).abs() < 60.0, "hotspot y {}", rep.hot_y_um);
...
fn analog_pa_delta_t_scales_with_power() {
    ...
    assert!((rise2 / rise1 - 2.0).abs() < 1e-3, "rise {rise1} -> {rise2} not ~2×");
}
